...
void AmericanMahjong::print_tiles(int player) {
    vector<Tiles*> tehuda;
    if (player == 0) {
        cout  << "プレーヤー: ";
        tehuda = myCards;
    ...
    int length = static_cast<unsigned int>(tehuda.size());
    int i=0;
    while(i<length){
        print_tile(tehuda[i]);
        cout << ' ';
...
void AmericanMahjong::start_game(Wind wind){
    ...
    tiles_left += i; // iが0以外の場合実際yamaに入ってるタイルの数に到達する前にゲームが終わってしまうためiをtiles_leftに加算する
...
int AmericanMahjong::evaluate_yaku(int i){
    ...
    bool thereIsJoker = false;
    if(num_each_tiles[11] != 0) thereIsJoker = true;
    
    bool didPlayerPon = false;
    if(myPons.size() != 0) didPlayerPon = true;
